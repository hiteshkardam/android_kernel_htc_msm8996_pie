			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: htc_effect: index %d enable %d return %s\n", __func__, index, enable_flag, ret ? "success" : "error!");
	...
	if (index == (htc_as10_offset + 3)) {
		...
		if (payload_int) {
			...
			*(payload_int + 11 + ADAP_ONE_CHANNEL_CONF_NUM_BY_DWORD) = (int)ucontrol->value.bytes.data[0];
			...
			if (!ret)
				pr_debug("%s: htc_effect set AS error on index:%d \n", __func__, index);
		}
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s, value %d, htc_as20_volume_index %d\n", __func__ , value, htc_as20_volume_index);
...
static int msm_htc_as20_vol_index_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s, htc_as20_volume_index %d\n", __func__ , htc_as20_volume_index);
...
static int headset_dt_parser(struct platform_device *pdev, struct wcd_mbhc_config *wcd_mbhc_cfg){
	...
	for (i = 0; i < id_gpio_count; i++){
		...
		if (gpio_is_valid(wcd_mbhc_cfg->htc_headset_cfg.id_gpio[i])) {
			pr_debug("%s: gpio %s parse success gpio no %d\n", __func__,
...
static int msm8996_setparam(htc_adsp_params_ioctl_t *ctrl)
{
	...
	if (htc_payload[index].payload) {
		...
		if (!ret)
			pr_debug("%s: htc_effect set AS error on index:%d return fail!\n", __func__, index);
	}
	mutex_unlock(&htc_adaptivesound_enable_mutex);
...
/* HTC_AUD_START */
	if((apr_get_q6_state() == APR_SUBSYS_LOADED) && card_reg == -1) {
		pr_debug("%s: schedule_delayed_work with card_det_work\n", __func__);
...
/* HTC_AUD_START */
	...
	if (pdata->audio_1v8_hph_en_gpio < 0) {
		...
		if (ret < 0) {
			...
			pr_debug("%s: audio-1v8-hph-en output high\n", __func__);
		}
	}
	...
	if (pdata->rt5503_reset_gpio < 0) {
		...
		if (ret < 0) {
			...
			pr_debug("%s: rt5503_reset_gpio output high\n", __func__);
		}
	}
#endif
...
/* HTC_AUD_START */
	pr_debug("%s:  cancel_delayed_work with card_det_work\n", __func__);
...
/* HTC_AUD_START */
	...
	if (ret) {
		pr_debug("%s: Missing htc-tfa9888 in dt node\n", __func__);
		tfa9888_dev = 0;
	} else {
		pr_debug("%s: htc-tfa9888 is %s in dt node\n", __func__, tfa9888_dev ? "true" : "false");
		if (tfa9888_dev) {
	...
	if (ret) {
		pr_debug("%s: Missing htc-rt5503 in dt node\n", __func__);
		rt5503_dev = 0;
	} else {
		pr_debug("%s: htc-rt5503 is %s in dt node\n", __func__, rt5503_dev ? "true" : "false");
		if (rt5503_dev) {
